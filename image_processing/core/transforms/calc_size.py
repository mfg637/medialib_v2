...
def scale_down_to_fill(
    ...
    if height_orig <= width_orig:
        ...
        if it_fits_in(height_orig, min_size):
            width_scaled = int(bit_round(width_orig, size_precision))
            height_scaled = int(bit_round(height_orig, size_precision))
        else:
...
def check_size_fills_in(
    ...
    """
    return (
        current_size[0] <= max_side_size[0]
        and current_size[1] <= max_side_size[1]
    )
